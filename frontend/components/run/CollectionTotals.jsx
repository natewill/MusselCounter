export default function CollectionTotals({ collection, imageCount, images }) {
  ...
  // Use calculated totals if we have processed images, otherwise fall back to collection totals
  const displayLiveCount = images && images.length > 0 ? liveCount : (collection.live_mussel_count || 0);
  const displayDeadCount = images && images.length > 0 ? deadCount : (collection.dead_mussel_count || 0);
