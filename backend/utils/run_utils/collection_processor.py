...
async def _prepare_images(db: aiosqlite.Connection, collection_id: int, run_id: int):
    """
    Get images from collection, filter duplicates, and check which are already processed.
    
    Returns:
        Tuple of (images_to_process, all_images, already_processed)
    ...
    return (images_to_process, all_images, already_processed)


async def _handle_all_images_processed(db: aiosqlite.Connection, run_id: int, collection_id: int, all_images: list):
    """
...
async def process_collection_run(db: aiosqlite.Connection, run_id: int):
    ...
    try:
        ...
        images_to_process, all_images, already_processed = image_prep
        
        if not images_to_process:
            # All images already processed
            await _handle_all_images_processed(db, run_id, collection_id, all_images)
            return
            ...
            async def process_batch_of_images(batch_images, batch_idx):
                ...
                async with semaphore:
                    ...
                    for (image_id, _, _), result in zip(image_data, results):
                        ...
                        # Save detections to database for threshold recalculation
                        try:
                            await _save_detections_to_db(db_path, run_id, image_id, result)
                        except Exception as e:
                            logger.error(f"Failed to save detections for image {image_id}: {e}", exc_info=True)
                            # Continue processing even if detection saving fails
